...
var CreateNewsCategory = function () {
    ...
    if (model.Order < 0) {
        alert("Thứ tự không thể nhỏ hơn 0");
        return;
    }
...
var GetAPIType = function () {
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i = start; i < range; i++) {
                $("#apitype-table-body").append("<tr>"
                    + "<td class='text-center'>"
                    //+ "<input type='checkbox' onclick value='" + data[i].APITypeId + "' class='newsCategoryMultiSelect'>"
                    + "</td>"
                    + "<td class='text-center'>"
                    + data[i].Name
                    + "</td>"
                    ...
                    + "</td>"
                    + "<td class='text-center'>"
                    + "<button type='button' onclick='EditAPIType(this)' value='" + data[i].APITypeId + "' data-toggle='modal' data-target='#APITypeModal' style='background-color:deepskyblue;width:28px;height:28px;padding:4px;margin:5px' class='btn'><i style='font-size:20px;color:white' class='fa fa-edit'></i></button>"
                    + "<button type='button' onclick='PrepareDeleteAPIType(this)' data-toggle='modal' data-target='#myModal' value='" + data[i].APITypeId + "' style='background-color:red;width:28px;height:28px;padding:4px;' class='btn'><i style='font-size:20px;color:white' class='fa fa-trash'></i></button>"
                    + "</td>"
                    + "</tr>"
                );
            }
...
var APIType_GetQuantity = function () {
    $.ajax({
        url: "/api/v1/newscategory/quantity",
        dataType: "json",
        method: "GET",
        success: function (data) {
            ...
            $("#APIType_TotalPage").html(TotalPage);
            $("#APIType_PageNumber").val(PageNumber);
            APIType_Pagination();
        },
        error: function (response) {
            console.log(response);
...
//PAGINATION
var APIType_NextPage = function () {
    previousPage = PageNumber;
    PageNumber++;
    APIType_GoToPage(PageNumber);
    //GetData();

};

var APIType_PreviousPage = function () {
    if (PageNumber > 1) {
        previousPage = PageNumber;
        PageNumber--;
        APIType_GoToPage(PageNumber);
...
$("#APIType_NextPage").click(function () {
    APIType_NextPage();
});

$("#APIType_PreviousPage").click(function () {
    APIType_PreviousPage();
});
var APIType_Pagination = function () {
    $("#APIType_Pages").html("");
    var show = 5;
    //var totalPages = Math.floor(TotalQuantity / Filter.PageSize < 5);
    //if (TotalQuantity / Filter.PageSize < 5) {
    ...
    if (PageNumber < 3) {
        for (var i = 1; i <= show; i++) {
            if (i == PageNumber) {
                $("#APIType_Pages").append("<button class='btn-primary' onclick='NewsCategory_GoToPage(" + i + ")' type='button'>" + i + "</button>");
            }
            else {
                $("#APIType_Pages").append("<button onclick='NewsCategory_GoToPage(" + i + ")' type='button'>" + i + "</button>");
            }
        }
    }
    else {
        for (var i = -2; i < show - 2; i++) {
            if (i == 0) {
                $("#APIType_Pages").append("<button class='btn-primary' onclick='APIType_GoToPage(" + (parseInt(PageNumber) + i) + ")' type='button'>" + (PageNumber + i) + "</button>");
            }
            else {
                $("#APIType_Pages").append("<button onclick='APIType_GoToPage(" + (parseInt(PageNumber) + i) + ")' type='button'>" + (PageNumber + i) + "</button>");
            }
        }
    }
};

var APIType_GoToPage = function (pageNumber) {
    if (pageNumber <= TotalPage) {
        previousPage = PageNumber;
        PageNumber = pageNumber;
        GetAPIType();
...
$("#APIType_PageNumber").keyup(function (e) {
    PageNumber = parseInt($("#APIType_PageNumber").val());
    if (e.keyCode == 13) {
        APIType_GoToPage(PageNumber);
    }
});

$("#APIType_BtnSave").click(function () {
    if (isCreated) {
        CreateAPIType();
    }
    else {
...
$("#APIType_BtnReset").click(function () {
    $('#APITypeModal').modal('hide');
    $("#APIType_Name").val('');
    $("#APIType_Order").val('')
});

var CreateAPIType = function () {
    var model = {};
    model.Name = $("#APIType_Name").val();
    if (model.Name == "" || model.Name == null || model.Name == undefined) {
        alert("Tên loại API không được bỏ trống");
        return;
    }
    model.Order = parseInt($("#APIType_Order").val());
    if (model.Order < 0) {
        alert("Thứ tự không thể nhỏ hơn 0");
        return;
    }
    ...
    $.ajax({
        url: "/api/v1/APIType/create",
        method: "POST",
        data: model,
        success: function (data) {
            alert("Thêm mới loại API thành công");
            GetAPIType();
        },
        error: function () {
            alert("Thêm mới loại API thất bại. Không thể kết nối tới máy chủ.")
        }
    });
    
}

$("#APITypeCreate").click(function () {
    isCreated = true;
});
var EditAPIType = function (btn) {
    globalId = btn.value;
    isCreated = false;
    var model = {};
    $.ajax({
        url: '/api/v1/APIType/?APITypeId=' + globalId,
        method: "GET",
        dataType: "json",
        success: function (data) {
            model = data;
            $('#APITypeModal').modal('show');
            $("#APIType_Name").val(model.Name);
            $("#APIType_Order").val(model.Order);
        },
        error: function (data) {
            alert("Có lỗi xảy ra khi lấy dữ liệu từ máy chủ");
        }
...
var UpdateAPIType = function () {
    var model = {};
    model.APITypeId = globalId;
    model.Name = $("#APIType_Name").val();
    if (model.Name == "" || model.Name == null || model.Name == undefined) {
        alert("Tên loại API không được bỏ trống");
        return;
    ...
    $.ajax({
        url: "/api/v1/APIType/update",
        method: "PUT",
        data: model,
        success: function (data) {
            alert("Cập nhật loại API thành công");
            $("#APIType_BtnReset").click();
            GetAPIType();
        },
        error: function () {
            alert("Cập nhật loại API thất bại. Không thể kết nối tới máy chủ.")
        }
    });
    
}

var PrepareDeleteAPIType = function (btn) {
    var id = btn.value;
    if (confirm("Xác nhận xóa loại API?")) {
        DeleteAPIType(id);
    }
}

var DeleteAPIType = function (id) {
    $.ajax({
        url: "/api/v1/APIType/delete?id=" + id,
        method: "DELETE",
        success: function (data) {
            alert("Xóa loại API thành công!");
            $("#APIType_BtnReset").click();
            GetAPIType();
        },
        error: function (data) {
            alert("Xóa loại API thất bại");
        }
    })
};